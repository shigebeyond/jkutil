...
internal interface IJedisFactory {
    ...
    fun buildJedisPoolConfig(config: Config): JedisPoolConfig {
        ...
        poolConfig.timeBetweenEvictionRunsMillis = config.getLong("timeBetweenEvictionRunsMillis", 1000 * 60 * 10)!! //表示idle object evitor两次扫描之间要sleep的毫秒数
        poolConfig.numTestsPerEvictionRun = config.getInt("numTestsPerEvictionRun", 10)!! //表示idle object evitor每次扫描的最多的对象数
        poolConfig.minEvictableIdleTimeMillis = config.getLong("minEvictableIdleTimeMillis", 1000 * 60 * 60)!! //表示一个对象至少停留在idle状态的最短时间，然后才能被idle object evitor扫描并驱逐；这一项只有在timeBetweenEvictionRunsMillis大于0时才有意义
        return poolConfig
    }
}